	...
	synchronized (parent.getTreeLock())
	{
		...
		for (Component component: parent.getComponents())
		{
			if (component.isVisible())
			{
				...
				if(minX == null || location.x < minX)
					minX = location.x;
				if(minY == null || location.y < minY)
					minY = location.y;
			}
		}
		...
		int moveY = 0;
		if(minX != null)
			moveX = parentInsets.left - minX;
		if(minY != null)
			moveY = parentInsets.left - minY;



		//minY = (minY < parentInsets.top) ? parentInsets.top - minY : 0;
		// System.out.println("off "+leftOffset);
		...
		for (Component component: parent.getComponents())
		{
			if (component.isVisible())
			{
				...
				// System.out.println("bufx "+bufx);
				component.setBounds(p.x + moveX, p.y + moveY, d.width, d.height);
			}
		}
		leftOffset += moveX;
		topOffset += moveY;
